...
#### 方式一：GitHub Actions 云编译（推荐，无需 Mac）

仓库内置流水线 `.github/workflows/build-ipa.yml`，在 GitHub 的 macOS 云主机上编译并产出
**未签名 IPA**，可直接用 TrollStore（巨魔）安装。

1. 把本仓库推送到你自己的 GitHub 仓库；
2. 打开仓库的 **Actions** 页面 → 左侧选 **Build unsigned IPA (TrollStore)** → 点 **Run workflow**；
3. 等约 5~15 分钟（首次需要下载 VLCKit 二进制包，会久一些）；
4. 构建成功后，在本次运行的 **Artifacts** 区域下载 `TVBox-unsigned-ipa`，解压得到 `TVBox-unsigned.ipa`。

> 推送代码到 `main` / `master` 分支也会自动触发构建。
...
3. **注意**：
   - `package_ios.sh` 走的是「证书 + 描述文件」的正式签名流程，需要 `IOS-key/secrets.sh` 等私有文件，普通用户无法直接使用。
   - 只想拿未签名包给 TrollStore 用，请走上面的 GitHub Actions 流程。
