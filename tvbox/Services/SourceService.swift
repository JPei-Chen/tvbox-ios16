...
class SourceService {
    ...
    /// 构建接口请求地址。
    ///
    /// 除常规拼接外，还要识别一种常见的「代理订阅」形态：
    /// `https://proxy.example.com/?url=https://real.host/api.php/provide/vod`
    /// 这类代理把 `url=` 之后的**整段**当作目标地址转发，
    /// 因此在外层 query 上追加参数会被直接丢弃（`?url=X&ac=list` 还会被拼成非法路径而 404）。
    /// 正确做法是把参数拼到内层 URL 上：
    /// `https://proxy.example.com/?url=https://real.host/api.php/provide/vod?ac=videolist&pg=1`
    private func buildURL(base: String, queryItems: [URLQueryItem]) throws -> String {
        let trimmedBase = base.trimmingCharacters(in: .whitespacesAndNewlines)
        guard let components = URLComponents(string: trimmedBase) else {
            throw SourceError.invalidApiUrl(base)
        }
        
        if let proxyURL = Self.buildURLWithNestedProxy(base: components, queryItems: queryItems) {
            return proxyURL
        }
        
        guard var directComponents = URLComponents(string: trimmedBase) else {
            throw SourceError.invalidApiUrl(base)
        }
        
        var mergedQueryItems = directComponents.queryItems ?? []
        mergedQueryItems.append(contentsOf: queryItems)
        directComponents.queryItems = mergedQueryItems
        
        guard let url = directComponents.url else {
            throw SourceError.invalidApiUrl(base)
        }
        return url.absoluteString
    }
    
    /// 识别 `?url=<真实接口>` 形式的代理地址并以内层拼接方式构造请求。
    ///
    /// 返回 nil 表示当前地址不是这种代理，调用方应回退到常规拼接。
    /// 判定条件较严格（存在且仅存在一个名为 url 的 http(s) 参数），
    /// 避免把普通业务参数误判为代理地址。
    private static func buildURLWithNestedProxy(
        base: URLComponents,
        queryItems: [URLQueryItem]
    ) -> String? {
        guard let items = base.queryItems, !queryItems.isEmpty else { return nil }
        
        let urlItems = items.filter { $0.name.lowercased() == "url" }
        guard urlItems.count == 1,
              let inner = urlItems[0].value?.trimmingCharacters(in: .whitespacesAndNewlines),
              inner.hasPrefix("http://") || inner.hasPrefix("https://"),
              var innerComponents = URLComponents(string: inner) else {
            return nil
        }
        
        // 参数一律拼进内层接口，由内层服务器处理。
        var innerItems = innerComponents.queryItems ?? []
        innerItems.append(contentsOf: queryItems)
        innerComponents.queryItems = innerItems
        guard let mergedInner = innerComponents.url?.absoluteString else { return nil }
        
        // 外层保留除 url 以外的参数，并把 url 放到最后：
        // 该类代理以「url= 之后直到结尾」作为目标地址，url 后面不能再跟其他参数。
        var outer = ""
        if let scheme = base.scheme { outer += scheme + "://" }
        outer += base.host ?? ""
        if let port = base.port { outer += ":" + String(port) }
        let path = base.percentEncodedPath
        outer += path.isEmpty ? "/" : path
        
        var parts: [String] = []
        for item in items where item.name.lowercased() != "url" {
            parts.append(item.name + "=" + (item.value ?? ""))
        }
        parts.append("url=" + mergedInner)
        outer += "?" + parts.joined(separator: "&")
